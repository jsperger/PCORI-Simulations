...
# https://nclottery.com/Pick4 Thursday 12-10-20 Daytime Draw
sim.seed <- 3933
...
#################################################
# Treatment effects
param.df <- read.csv("./Settings/full_null.csv")
...
true.model.formula <- formula(~ 1 + (Dwell + Music )^2 + B1:Dwell + B1:Music + FirstCysto*Music)


mixed.model.string <- "Yij ~ (1|ID) + (Dwell + Music)^2  + B1:Dwell + B1:Music + FirstCysto*Music"
gee.model.formula <- formula(Yij ~ 1 + (Dwell + Music)^2  + B1:Dwell + B1:Music + FirstCysto*Music)
...
##
################################################################################
# Exclude the intercept and non-treatment main effects
contrast.mat <- diag(8)[-c(1, 4),]
rownames(contrast.mat) <- c("Dwell", "Music", "Dwell:Music", "Dwell:B1", "Music:B1", "Music:FirstCysto")
